...
r"""Convert raw labelar-COCO dataset to TFRecord for object_detection.

Please note that this tool creates sharded output files.

Example usage:
    python create_labelar_tf_record.py --logtostderr \
...
tf.flags.DEFINE_string("ds_name", "", "name of dataset")
FLAGS = flags.FLAGS
...
def _create_labelmap_from_coco_annotations(
    ann_path: Path, label_output_path: Path
) -> None:
    assert ann_path.exists(), f"Annotation file not found: '{ann_path}'"
    assert ann_path.is_file(), f"Annotation path is not a file: '{ann_path}'"
    coco = COCO(ann_path)
...
def main(_):
    ...
    train_output_path = os.path.join(FLAGS.output_dir, f"{FLAGS.ds_name}_train.record")
    val_output_path = os.path.join(FLAGS.output_dir, f"{FLAGS.ds_name}_val.record")
    ...
    #     num_shards=100,
    # )
    _create_labelmap_from_coco_annotations(
        Path(FLAGS.train_annotations_file),
        Path(FLAGS.output_dir) / f"{FLAGS.ds_name}_label_map.pbtxt",
    )
