...
def validate_dataset(filenames, reader_opts=None):
    ...
    for fname in filenames:
        ...
        record_iterator = tf.python_io.tf_record_iterator(
            path=fname, options=reader_opts
        )
        try:
        ...
        with open(f_name, "rb") as f:
            ...
            while len(len_bytes) > 0:
                ...
                length, = struct.unpack(
                    "<Q", len_bytes
                )  # u64: length of the protobuf data (excluding the header)
                len_crc, = struct.unpack(
                    "<I", f.read(4)
                )  # u32: masked crc32c of the length bytes
                data = f.read(length)  # protobuf data
                data_crc, = struct.unpack(
                    "<I", f.read(4)
                )  # u32: masked crc32c of the protobuf data
